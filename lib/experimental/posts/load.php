...
function gutenberg_posts_dashboard() {
	...
	wp_add_inline_script( 'wp-edit-site', 'window.wp.editSite.initializePostsDashboard( "gutenberg-posts-dashboard" );' );
...
function gutenberg_replace_posts_dashboard() {
	...
	add_menu_page( $ptype_obj->labels->name, $ptype_obj->labels->name, $ptype_obj->cap->edit_posts, "gutenberg_posts_dashboard", "gutenberg_posts_dashboard", "dashicons-admin-post", 6 );
}
